...
For each prescription below, the recommended path is the Realize UI navigation. After the user confirms they've made the change, re-verify via the MCP read tools (`get_campaign` / `get_campaign_items` / `get_campaign_history_report`) and re-pull the relevant report after a data window (typically 3–7 days, accounting for the change-class lag in `references/optimization-flow.md`).
