...
Brand safety ensures ads don't appear alongside content that could damage your brand. Realize offers native tools (keyword blocking, topic targeting, publisher lists), third-party pre-bid integrations (DoubleVerify, IAS), and verification tags. This file covers all mechanisms and undesirable-placement handling.
...
| **Keyword Blocking** | Negative keyword targeting restricts the campaign from showing on pages with specific keywords in the title or URL | Example: Pepsi can block any page mentioning "Coke." Provide your list of terms / keywords. |
...
|---|---|---|
| **Approved Site Lists** | Specify which sites the campaign runs on | May significantly impact scale. |
| **Blocked Site Lists** | Specify which sites to exclude | May impact scale — monitor delivery. |
...
| Documentation | **Have on hand** the DV blueprint or IAS media plan before setup — it contains all pre-bid requirements. |
...
| **JavaScript tags** | If brand-safety and viewability monitoring via JS tags is in scope, expect a reduction in scale. API publishers (Yahoo!, MSN) do not accept JavaScript tags — ads will not serve on those environments when JS tags are applied. |
...
- Never add brand-safety restrictions without surfacing the scale impact.
- Never dismiss placement concerns — always investigate and take action.
- Never exclude sites without meeting the data thresholds (Campaign Clicks ≥ 500, Conversions ≥ 5).
- Always have the DV blueprint or IAS media plan on hand before setup.
- Always verify language support (Hebrew is not supported by DV).
- Always factor in that JavaScript tags reduce scale (API publishers like Yahoo / MSN do not accept them).
...
4. **JavaScript tags without scale warning.** Yahoo / MSN won't serve ads. Plan for reach reduction.
...
- Adding more brand-safety selections limits supply availability — weigh the trade-off between safety and scale before applying.
